...
class RegistrationsController < Devise::RegistrationsController
  before_action :authenticate_user!, except: %i[do_reset_password reset_password_edit]
    ...
    respond_to do |format|
      format.html do
        redirect_to '/users/edit',
                    notice: 'You will receive an email with instructions about how to reset your password in a few minutes.'
      end
    end
  end
  ...
  def do_reset_password
    id = params[:id]
    id = current_user.id if id.nil? && !current_user.nil?

    @user = if id.nil?
              User.where(reset_password_token: params[:user][:reset_password_token]).first
            else
              User.find(id)
            end
